...
const CardReviews = ({ name, rating, date, comment }) => {
  ...
  return (
    <FadeIn>
      <div className="flex flex-col p-6 h-full border rounded-xl transition-all duration-300 bg-white/5 backdrop-blur-sm border-white/10 hover:border-violet-400/30">
        ...
        <p className="text-sm text-indigo-200 mb-4">{formatDate(date)}</p>
        ...
        <div className="flex-grow">
          ...
          {shouldShowExpand && (
            <button
              ...
              className="text-sm font-medium transition-colors duration-200 mt-3 text-white hover:text-violet-300"
              aria-label={`${
